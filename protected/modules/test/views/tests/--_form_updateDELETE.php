...
<!--
          ...
          <div class="qa-form">s
          <!--  тут так то надо делать все через form-> но я хз как правильно. не смог убрать лейбел у текстареа       --!>
          ...
          <?php /* echo CHtml::link('<i class="icon-trash"></i>', '#', array('class' => 'delete delete-question','rel'=>'tooltip','data-original-title'=>'Удалить вопрос'));
                 $numberQpl1=$numberQ+1;
                 echo CHtml::encode('Вопрос # '."$numberQpl1");
                 echo $form->textAreaRow($question,'statement',array('class'=>'span8','maxlength'=>64,'labelOptions'=>array('label'=>""),'name'=>"Question[$numberQ]"));
                 echo CHtml::tag('label',array(),'Варианты ответов:');
            //*/
 ?>
</div>
           <?
        ...
        //
         /*
            $gridDataProvider = new CArrayDataProvider(array(
                ...
                array('id'=>3, 'firstName'=>'Stu', 'lastName'=>'Dent', 'language'=>'sd'),
            ));
             //*/
          // echo var_dump($gridDataProvider);
          /*
           $gridDataProvider = new CArrayDataProvider($question->answers,array('keyField'=>'id','questionN'=>'fk_question'));
          // echo "<br>----<br>";
          // echo var_dump($gridDataProvider);
                ...
                $this->widget('bootstrap.widgets.TbGridView', array(
                   'type'=>'striped bordered condensed',
                    'dataProvider'=>$question->answers->search(),
                    'template'=>"{items}",
                    'columns'=>array(
                        array('type' => 'raw','value'=>'CHtml::textField( "Text", "some value",array("width"=>"1000"))', 'header'=>'Вариант ответа'.'<button class="btn btn-mini btn-primary add-answer" type="button">Добавить</button>'),
                        array( 'name'=>'id' ,'type' => 'raw','value'=>'CHtml::textField( "Text","",array("width"=>"1000"))','header'=>'Балл',),
                        array(
     ...
     ?>
                 ...
                 <table class="table table-bordered table-condensed answer-table" max_answer_number="<? $max_key_number = count($question->answers)-1;  echo $max_key_number ?>" question_number="<? echo $numberQ ?>">
                    <thead>
                      <tr>
...
<br />
	   ...
	   <hr />
        ...
        <div class="form-actionsd" id="template-qa-field">
                <label>Вопрос</label>
                ...
                <thead>
                  ...
                  <tr a_number="2">
                    ...
                    <td>
                        <a class="delete delete-answer-row" rel="tooltip" href="#" data-original-title="Удалить" ><i class="icon-trash"></i></a>
                        <a class="delete delete-answer-row" rel="tooltip" href="#" data-original-title="Удалить" ><i class="icon-trash"></i></a>
                    </td>
                  </tr>
                </table>
